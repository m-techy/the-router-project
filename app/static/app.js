const state={providers:[],usage:[],health:null,setup:null,snippets:null,activeSnippet:'python'};
...
async function refresh(){try{const [health,p,usage,summary,setup,snippets]=await Promise.all([api('/health'),api('/api/providers'),api('/api/usage/recent?limit=100'),api('/api/usage/summary?hours=24'),api('/api/setup/status'),api('/api/setup/snippets')]);state.health=health;state.providers=p.providers;state.usage=usage.events;state.setup=setup;state.snippets=snippets;$('#healthDot').style.background='#63d9a5';$('#healthLabel').textContent=`${health.configured_providers}/${health.providers} providers configured`;renderOverview();renderSetup();renderProviders();renderUsage(summary)}catch(e){$('#healthDot').style.background='#ff7b8b';$('#healthLabel').textContent='Router unavailable'}}
